...
export default function NvkLichNhanVienThem() {
  ...
  useEffect(() => {
    if (user?.role === 'nhanvien') {
      setFormData(prev => ({ ...prev, G5_MaNhanVien: user.id }));
    }
  }, [user]);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-4">
          ...
          <input 
            ...
            disabled={user?.role === 'nhanvien'}
            className={`w-full p-2 rounded border focus:ring-2 focus:ring-blue-500/50 outline-none transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'} ${user?.role === 'nhanvien' ? 'opacity-65 cursor-not-allowed' : ''}`} 
          />
        </div>
