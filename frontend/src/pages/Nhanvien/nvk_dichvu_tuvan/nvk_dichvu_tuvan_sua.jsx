...
export default function NvkDichVuTuVanSua() {
  ...
  return (
    <div className={`p-6 min-h-screen font-['Inter'] ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Sửa Dịch Vụ Tư Vấn</h1>
          <p className="text-xs text-slate-500 mt-1">Cập nhật thông tin gói dịch vụ tư vấn mã #{id}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className={`p-6 rounded-2xl shadow-xl border space-y-5 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          <div>
            <label className={`block text-xs font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Tên Dịch Vụ</label>
            <input
              type="text"
              name="G5_TenDichVu"
