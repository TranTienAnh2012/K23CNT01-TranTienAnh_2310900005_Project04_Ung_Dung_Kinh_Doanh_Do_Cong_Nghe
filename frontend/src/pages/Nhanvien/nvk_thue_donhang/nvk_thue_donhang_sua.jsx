...
export default function NvkThueDonHangSua() {
  ...
  useEffect(() => {
    if (true && id) {
      donhangThueApi.getAll().then(res => {
        if(res.data?.success) {
          const items = res.data.data.items || res.data.data;
          const item = items.find(x => String(x.G5_MaDonThue) === String(id));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (true) {
        await donhangThueApi.update(id, formData);
      } else {
        await donhangThueApi.create(formData);
      }
      navigate('/admin/donhang-thue');
    } catch (err) {
  ...
  return (
    <div className={`p-6 min-h-screen ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      <div className="max-w-2xl mx-auto">
        <h1 className={`text-2xl font-bold mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>Sửa Đơn Hàng Thuê</h1>
        ...
        <div className="mb-4">
          <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Tổng Tiền</label>
          <input type="text" name="G5_TongTien" value={formData.G5_TongTien || ''} onChange={handleChange} className={`w-full p-2 rounded border focus:ring-2 focus:ring-blue-500/50 outline-none transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} />
        </div>
        <div className="mb-4">
          <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Trạng Thái</label>
          <select 
            name="G5_TrangThai" 
            value={formData.G5_TrangThai || ''} 
