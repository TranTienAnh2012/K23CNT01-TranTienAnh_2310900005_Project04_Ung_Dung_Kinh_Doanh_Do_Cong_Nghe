...
export default function NnhGioHangTamThem() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (false) {
        await giohangTamApi.update(id, formData);
      } else {
        await giohangTamApi.create(formData);
      }
      navigate('/admin/giohang-tam');
    } catch (err) {
  ...
  return (
    <div className={`p-6 min-h-screen ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      <div className="max-w-2xl mx-auto">
        <h1 className={`text-2xl font-bold mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>Thêm Giỏ Hàng Tạm</h1>
