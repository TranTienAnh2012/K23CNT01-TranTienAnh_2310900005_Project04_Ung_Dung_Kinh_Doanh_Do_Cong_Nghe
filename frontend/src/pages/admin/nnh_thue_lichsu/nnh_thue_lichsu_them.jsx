...
export default function NnhThueLichSuThem() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (false) {
        await lichsuThueApi.update(id, formData);
      } else {
        await lichsuThueApi.create(formData);
      }
      navigate('/admin/lichsu-thue');
    } catch (err) {
  ...
  return (
    <div className={`p-6 min-h-screen ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      <div className="max-w-2xl mx-auto">
        <h1 className={`text-2xl font-bold mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>Thêm Lịch Sử Thuê</h1>
        ...
        <div className="mb-4">
          <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Thời Điểm</label>
          <input type="text" name="G5_ThoiDiem" value={formData.G5_ThoiDiem || ''} onChange={handleChange} className={`w-full p-2 rounded border focus:ring-2 focus:ring-blue-500/50 outline-none transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} />
        </div>
