...
export default function NnhThueLichSuList() {
  ...
  return (
    <div className={`p-6 min-h-screen ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      <div className="flex justify-between items-center mb-6">
        <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Lịch Sử Thuê</h1>
        <Link to="/admin/lichsu-thue/them" className="bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700 transition-colors">
          + Thêm Mới
        </Link>
      </div>
        ...
        <table className="min-w-full">
          <thead className={isDark ? 'bg-slate-800/50' : 'bg-gray-50'}>
            <tr>
                ...
                <th className={`px-4 py-3 border-b text-left text-xs font-semibold uppercase tracking-wider ${isDark ? 'text-slate-300 border-slate-700' : 'text-slate-600 border-slate-200'}`}>Thời Điểm</th>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.G5_Id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-gray-50'}`}>
                ...
                <td className={`px-4 py-3 border-b ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>{item.G5_ThoiDiem}</td>
                <td className={`px-4 py-3 border-b text-right ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                  <Link to={`/admin/lichsu-thue/edit/${item.G5_Id}`} className="text-blue-500 hover:text-blue-400 mr-4 font-medium">Sửa</Link>
