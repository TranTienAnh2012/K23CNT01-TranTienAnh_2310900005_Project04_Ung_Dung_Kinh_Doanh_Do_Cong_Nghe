    ...
    <div className={`p-8 min-h-[calc(100vh-64px)] ${isDark ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-800'} transition-colors duration-300 font-['Inter']`}>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-8 gap-4">
        <div>
          ...
          <h2 className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} font-['Space_Grotesk'] tracking-tight`}>Quản lý Thuộc tính Gốc</h2>
          <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} text-sm mt-1`}>Định nghĩa các loại thuộc tính cơ bản (Màu sắc, Dung lượng, CPU...) dùng chung cho toàn bộ hệ thống.</p>
        </div>
      ...
      <div className={`${isDark ? 'bg-slate-900/60 border-slate-800/50 shadow-black/50' : 'bg-white border-slate-200 shadow-xl'} backdrop-blur-md rounded-2xl overflow-hidden border shadow-2xl`}>
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {attributes.map((tt) => (
              <tr key={tt.ThuocTinhID} className={`${isDark ? 'hover:bg-white/[0.03]' : 'hover:bg-slate-50/80'} transition-colors group`}>
                ...
                <td className="px-8 py-5">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl ${tt.Bg || 'bg-slate-800'} flex items-center justify-center ${tt.Color || 'text-slate-400'}`}>
                      ...
                      <span className={`font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'} text-sm`}>{tt.TenThuocTinh}</span>
                    </div>
                </td>
        ...
        {/* Pagination */}
        <div className={`px-8 py-5 flex items-center justify-between ${isDark ? 'bg-slate-950/20 border-slate-800' : 'bg-slate-50/50 border-slate-100'} border-t`}>
          <span className="text-xs text-slate-500 font-medium">Hiển thị {attributes.length} thuộc tính</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
        <div className={`${isDark ? 'bg-slate-900/60 border-white/5 shadow-black/50' : 'bg-white border-slate-200 shadow-xl'} backdrop-blur-md p-6 rounded-2xl border shadow-2xl border-l-4 border-l-blue-500`}>
          <p className={`text-[10px] font-bold ${isDark ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-widest mb-1`}>Tổng thuộc tính</p>
          <p className={`font-['Space_Grotesk'] text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{attributes.length}</p>
        ...
        <div className={`${isDark ? 'bg-slate-900/60 border-white/5 shadow-black/50' : 'bg-white border-slate-200 shadow-xl'} backdrop-blur-md p-6 rounded-2xl border shadow-2xl`}>
          <p className={`text-[10px] font-bold ${isDark ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-widest mb-1`}>Dùng nhiều nhất</p>
          <p className={`font-['Space_Grotesk'] text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Dung lượng</p>
        ...
        <div className={`${isDark ? 'bg-slate-900/60 border-white/5 shadow-black/50' : 'bg-white border-slate-200 shadow-xl'} backdrop-blur-md p-6 rounded-2xl border shadow-2xl`}>
          <p className={`text-[10px] font-bold ${isDark ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-widest mb-1`}>Cấu hình Global</p>
          <p className={`font-['Space_Grotesk'] text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>--</p>
        ...
        <div className={`${isDark ? 'bg-slate-900/60 border-white/5 shadow-black/50' : 'bg-white border-slate-200 shadow-xl'} backdrop-blur-md p-6 rounded-2xl border shadow-2xl relative overflow-hidden group`}>
          ...
          <div className="relative z-10">
            ...
            <p className="font-['Space_Grotesk'] text-2xl font-bold text-emerald-400">NORMAL</p>
            <p className="mt-4 text-[10px] text-slate-500 font-mono tracking-tighter italic">Hệ thống đang hoạt động ổn định.</p>
          </div>
        </div>
      </div>
