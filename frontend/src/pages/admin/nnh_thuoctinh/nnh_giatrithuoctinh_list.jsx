...
export default function TtaGiaTriThuocTinhList() {
  ...
  const handleCategoryClick = (categoryName) => {
    // Map tên hiển thị sang tên danh mục thực tế trong DB nếu cần
    let dbCategory = categoryName;
    if (categoryName === 'Smartphone') dbCategory = 'Điện Thoại';
    if (categoryName === 'Laptop/PC') dbCategory = 'Laptop';
    
    navigate(`/admin/giatri-thuoctinh/them?category=${encodeURIComponent(dbCategory)}`);
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-10">
        <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl flex items-center gap-4 border border-white/5">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-white font-['Space_Grotesk']">{products.length}</h3>
          </div>
        </div>
        <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl flex items-center gap-4 border border-white/5">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-white font-['Space_Grotesk']">{values.length}</h3>
          </div>
        </div>
        <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl flex items-center gap-4 border border-white/5">
        ...
        <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl flex items-center gap-4 border border-white/5">
          ...
          <div>
            ...
            <h3 className="text-lg font-bold text-white font-['Space_Grotesk']">{new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</h3>
          </div>
        </div>
      </div>
        ...
        {values.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              <div 
                onClick={() => handleCategoryClick('Laptop/PC')}
                className="bg-slate-900/40 backdrop-blur-sm p-6 rounded-2xl w-64 text-left border border-dashed border-slate-700 hover:border-blue-500/50 transition-all cursor-pointer group hover:bg-blue-600/5"
              >
              ...
              <div 
                ...
                className="bg-slate-900/40 backdrop-blur-sm p-6 rounded-2xl w-64 text-left border border-dashed border-slate-700 hover:border-cyan-500/50 transition-all cursor-pointer group hover:bg-cyan-600/5"
              >
              ...
              <div 
                onClick={() => handleCategoryClick('Phụ kiện')}
                className="bg-slate-900/40 backdrop-blur-sm p-6 rounded-2xl w-64 text-left border border-dashed border-slate-700 hover:border-purple-500/50 transition-all cursor-pointer group hover:bg-purple-600/5"
              >
            ...
            <div className="mt-12">
              <button 
                ...
                className="text-blue-500 font-bold text-xs flex items-center gap-2 hover:underline transition-all"
              >
