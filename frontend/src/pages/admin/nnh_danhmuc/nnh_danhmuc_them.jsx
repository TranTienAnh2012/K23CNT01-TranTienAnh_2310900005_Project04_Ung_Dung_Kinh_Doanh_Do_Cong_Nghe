...
export default function NnhDanhMucThem() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await danhmucApi.create(formData);
      ...
      <div className={`w-full max-w-2xl ${isDark ? 'bg-slate-900/70 border-slate-800 shadow-black/50' : 'bg-white border-slate-200 shadow-xl'} backdrop-blur-md rounded-3xl overflow-hidden border shadow-2xl`}>
        <div className={`p-8 border-b ${isDark ? 'border-slate-800 bg-slate-900/40' : 'border-slate-100 bg-slate-50/50'}`}>
          <h3 className={`font-['Space_Grotesk'] text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'} flex items-center gap-3`}>
            <span className="material-symbols-outlined text-blue-500 text-3xl">create_new_folder</span>
            Thêm Danh Mục Mới
          </h3>
          <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'} mt-1`}>Tạo một nhóm phân loại sản phẩm mới cho hệ thống.</p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                placeholder="Thông tin giới thiệu về danh mục này..."
                value={formData.MoTa}
            ...
            <div className="flex items-center justify-between p-4 rounded-2xl border border-slate-800 bg-slate-950/30">
               <div>
                  <p className="text-sm font-bold text-slate-200">Kích hoạt ngay</p>
                  <p className="text-[10px] text-slate-500 uppercase font-bold tracking-tighter">Hiển thị danh mục trên cửa hàng</p>
               </div>
               <button 
                  type="button"
