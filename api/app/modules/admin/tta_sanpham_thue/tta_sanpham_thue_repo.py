...
def get_by_id(id):
    stmt = select(sanpham_thue).where(sanpham_thue.c.G5_Id == id)
    with engine.connect() as conn:
...
def create(data):
    stmt = insert(sanpham_thue).values(**data)
    with engine.connect() as conn:
        conn.execute(stmt)
        conn.commit()

def update_item(id, data):
    stmt = update(sanpham_thue).where(sanpham_thue.c.G5_Id == id).values(**data)
    with engine.connect() as conn:
        conn.execute(stmt)
        conn.commit()
