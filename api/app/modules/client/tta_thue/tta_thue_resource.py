...
from app.models.schema import sanpham_thue, donhang_thue, chitiet_donhang_thue, lich_su_thue, sanpham, user
from app.utils.helpers import response_success, response_error
from datetime import datetime
import decimal

class ClientSanPhamThueListResource(Resource):
    def get(self):
        stmt = select(
...
class ClientDonHangThueListResource(Resource):
    @jwt_required()
    def get(self):
    ...
    def post(self):
        ...
        stmt_check_user = select(user.c.G5_MaNguoiDung).where(user.c.G5_MaNguoiDung == int(user_id))
        with engine.connect() as conn:
            user_exists = conn.execute(stmt_check_user).fetchone()
            if not user_exists:
                return response_error("Tài khoản người dùng không tồn tại hoặc đã bị xóa. Vui lòng đăng xuất và đăng nhập lại.", 401)
...
class ClientDonHangThueCancelResource(Resource):
    @jwt_required()
    def put(self, id):
        user_id = get_jwt_identity()
